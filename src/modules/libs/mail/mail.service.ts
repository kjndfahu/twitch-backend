...
export class MailService {
	...
	public async sendVereficationToken(email: string, token: string){
		const domain = await this.configService.getOrThrow<string>('ALLOWED_ORIGIN')
		const html = await render(VerificationTemplate({domain, token}))

		return this.sendMail(email, 'Верефикация аккаунта', html)
	}
